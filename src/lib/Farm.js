...
class AutomationFarm {
  ...
  static __internal__buildMenu() {
    ...
    const useShovelTooltip =
      "Enabling this setting will use shovels to remove unwanted berries from plots." +
      Automation.Menu.TooltipSeparator +
      "Shovels will not be used if their is no available stock.";
    Automation.Menu.addLabeledAdvancedSettingsToggleButton(
      "Use shovels to remove unwanted berries",
      this.Settings.UseShovel,
      useShovelTooltip,
      farmingSettingPanel,
    );
  ...
  static __internal__farmLoop() {
    ...
    if (
      Automation.Utils.LocalStorage.getValue(this.Settings.UseShovel) === "true"
    ) {
  ...
  static __internal__plantAllBerries(berryToPlant) {
    if (this.__internal__freeSlotCount > 0) {
      const selectedBerryCount =
        App.game.farming.berryInventory[berryToPlant]();
    ...
    this.__internal__unlockStrategySelection.push({
      ...
      action: function () {
        ...
        if (
          ...
          App.game.farming.berryInventory[berryType]()
        ) {
  ...
  static __internal__addBerryRequirementBeforeFurtherUnlockStrategy(
    ...
    strategy.action = function () {
      ...
      for (const berryType of berriesToGather) {
        ...
        let neededAmount =
          berriesMinAmount - App.game.farming.berryInventory[berryType]();
        const berryHarvestAmount = BerryList[berryType].harvestAmount;
    ...
    strategy.setFoatingPanelContent = function () {
      const berriesNeeded = berriesToGather.filter(
        (berryType) =>
          App.game.farming.berryInventory[berryType]() < berriesMinAmount,
      );
   ...
   */
  static __internal__removeAnyUnwantedBerry(
    index,
    expectedBerryType,
    canUseShovel = Automation.Utils.LocalStorage.getValue(
      this.Settings.UseShovel,
    ) === "true",
  ) {
    const plot = App.game.farming.plotList[index];
    ...
    if (
      plot.berry === expectedBerryType ||
      (this.__internal__currentStrategy != null &&
        plot.berry === this.__internal__currentStrategy.berryToUnlock)
    ) {
      return false;
    }
  ...
  static __internal__setSlotConfigStrategy(step, berriesIndexes) {
    ...
    step.action = function () {
      const canUseShovel =
        Automation.Utils.LocalStorage.getValue(this.Settings.UseShovel) ===
        "true";
      // Remove any mutation that might have occured, as soon as possible
      ...
      const firstBerryBloomDuration =
        BerryList[firstBerryType].growthTime[PlotStage.Bloom];
        ...
        if (plot.berry !== expectedBerryType) {
          if (expectedBerryType === BerryType.None) {
            const berryBloomDuration =
              BerryList[plot.berry].growthTime[PlotStage.Bloom] - plot.age;
            ...
            const expectedBerryTime =
              BerryList[expectedBerryType].growthTime[PlotStage.Bloom];
      ...
      if (!waitBeforeStartingStrategy) {
        ...
        for (const berryType of berriesOrder) {
          const currentBerryBloomDuration =
            BerryList[berryType].growthTime[PlotStage.Bloom];
  ...
  static __internal__getBerryPlantingOrder(berriesIndexes) {
    return [...Object.keys(berriesIndexes)]
      ...
      .sort(
        (a, b) =>
          BerryList[b].growthTime[PlotStage.Bloom] -
          BerryList[a].growthTime[PlotStage.Bloom],
      );
  }
