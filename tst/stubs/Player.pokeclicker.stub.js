...
class Player {
  ...
  __initItemList() {
    // Init egg items
    for (const i of Object.keys(GameConstants.EggItemType).filter((eggType) =>
      isNaN(eggType)
    )) {
      this.__itemListCount[i] = 0;
      this.itemList[i] = function () {
        return this.__itemListCount[i];
      }.bind(this);
    }

    // Init fossil items
    for (const i of UndergroundItems.list
      .filter((it) => it.valueType === UndergroundItemValueType.Fossil)
      .map((x) => x.itemName)) {
      this.__itemListCount[i] = 0;
      this.itemList[i] = function () {
        return this.__itemListCount[i];
