...
class Breeding {
  ...
  gainEgg(egg) {
    ...
    throw AutomationTestUtils.formatErrors(
      `Error: Could not place ${EggType[egg.type]} Egg`
    );
  }
  ...
  getTypeCaughtStatus(type) {
    ...
    return hatchable.reduce(
      (status, pname) =>
        Math.min(status, PartyController.getCaughtStatusByName(pname)),
      CaughtStatus.CaughtShiny
    );
  }
  ...
  __getNextPokemonToHatch(type = null) {
    ...
    if (type != null) {
      ...
      for (const list of this.hatchList) {
        hatchable = hatchable.concat(
          list.slice(0, player.highestRegion() + 1).flat()
        );
      }
    }

    if (this.__hatchAnExistingPokemon) {
      return hatchable.filter(
        (pokemonName) =>
  ...
  __initEggList() {
    for (let i = 0; i < this.eggSlots; i++) {
      ...
      this.eggList[i] = function () {
        return this.__eggList[i];
      }.bind(this);
    }
  }
