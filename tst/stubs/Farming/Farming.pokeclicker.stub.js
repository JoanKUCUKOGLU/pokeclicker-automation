...
class Farming {
  ...
  canBuyPlot(index) {
    ...
    return App.game.wallet.hasAmount(
      new Amount(cost, GameConstants.Currency.farmPoint)
    );
  }
  ...
  __initBerryLists() {
    ...
    for (let i = 0; i < maxIndex; i++) {
      ...
      this.berryList.push(function () {
        return App.game.farming.__berryListCount[i];
      });
      this.unlockedBerries.push(function () {
        return App.game.farming.__isBerryUnlocked[i];
      });
    }
  }

  __initMutations() {
    for (let key of Object.keys(BerryType)
