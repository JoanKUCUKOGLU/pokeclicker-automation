...
class PokemonHelper {
  ...
  static getPokemonByName(name) {
    ...
    if (basePokemon === undefined) {
      throw AutomationTestUtils.formatErrors(
        `Cound not find pokemon named '${name}'`
      );
    }
  ...
  static calcNativeRegion(pokemonName) {
    ...
    const id = pokemon.id;
    const region = GameConstants.MaxIDPerRegion.findIndex(
      (maxRegionID) => maxRegionID >= Math.floor(id)
    );
  ...
  static __registerPokemon(
    ...
    if (pokemonMap[name] !== undefined) {
      throw AutomationTestUtils.formatErrors(
        `Trying to register pokemon named '${name}' twice`
      );
    }

    if (pokemonMap[id] !== undefined) {
      throw AutomationTestUtils.formatErrors(
        `Trying to register pokemon with the id '${id}' twice`
      );
    }

    pokemonMap[name] = new TmpPokemonListData(
      id,
      name,
      nativeRegion,
      catchRate,
