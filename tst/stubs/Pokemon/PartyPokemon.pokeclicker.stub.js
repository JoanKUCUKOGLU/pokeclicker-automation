...
class PartyPokemon {
  ...
  constructor(id, name, baseAttack, shiny) {
    ...
    this.name = name;
    this.shiny = shiny;
    this.proteinsUsed = function () {
      return this.__proteinsUsed;
    }.bind(this);
    this.heldItem = function () {
      return this.__heldItem;
    }.bind(this);
  ...
  calculateAttack(ignoreLevel = false) {
    ...
    return Math.max(
      1,
      Math.floor(
        (this.baseAttack * attackBonusMultiplier + this.attackBonusAmount) *
          levelMultiplier *
          evsMultiplier *
          heldItemMultiplier *
          shadowMultiplier
  ...
  clickAttackBonus() {
    ...
    const resistantBonus =
      this.pokerus >= GameConstants.Pokerus.Resistant ? 1 : 0;
    const purifiedBonus =
      this.shadow == GameConstants.ShadowStatus.Purified ? 1 : 0;
    const bonus = caughtBonus + shinyBonus + resistantBonus + purifiedBonus;

    const heldItemMultiplier =
      this.heldItem() instanceof HybridAttackBonusHeldItem
        ? this.heldItem().clickAttackBonus
        : 1;
  ...
  calculateEVAttackBonus() {
    ...
    return this.evs() < 50
      ? 1 + 0.01 * this.evs()
      : 1 + Math.min(1, Math.pow(this.evs() - 30, 0.075) - 0.75);
  }

  evs() {
    const power = App.game.challenges.list.slowEVs.active()
  ...
  shadowAttackBonus() {
    return this.shadow == GameConstants.ShadowStatus.Shadow
      ? 0.8
      : this.shadow == GameConstants.ShadowStatus.Purified
      ? 1.2
      : 1;
  }

  heldItemAttackBonus() {
    return this.heldItem && this.heldItem() instanceof AttackBonusHeldItem
