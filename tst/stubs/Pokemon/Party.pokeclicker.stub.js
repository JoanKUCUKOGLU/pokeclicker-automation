...
class Party {
  ...
  calculateOnePokemonAttack(
    ...
    let multiplier = 1;
    let attack = 0;
    const pAttack = useBaseAttack
      ? pokemon.baseAttack
      : ignoreLevel
      ? pokemon.calculateAttack(ignoreLevel)
      : pokemon.attack;
    ...
    // Weather boost
    const weather =
      Weather.weatherConditions[overrideWeather ?? Weather.currentWeather()];
    const dataPokemon = PokemonHelper.getPokemonByName(pokemon.name);
