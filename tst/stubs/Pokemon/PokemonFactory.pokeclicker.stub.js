...
class PokemonFactory {
  static catchRateHelper(baseCatchRate, noVariation = false) {
    ...
    const catchRateRaw =
      Math.floor(Math.pow(baseCatchRate, 0.75)) + catchVariation;
    return GameConstants.clipNumber(catchRateRaw, 0, 100);
  }

  static routeHealth(route, region) {
    const regionRoute = Routes.regionRoutes.find(
    ...
    const health =
      Math.max(
        20,
        Math.floor(
          Math.pow((100 * Math.pow(route, 2.2)) / 12, 1.15) * (1 + region / 20)
        )
      ) || 20;
    return health;
  }
  ...
  static routeDungeonTokens(route, region) {
    ...
    const tokens = Math.max(
      1,
      6 * Math.pow((route * 2) / (2.8 / (1 + region / 3)), 1.08)
    );
