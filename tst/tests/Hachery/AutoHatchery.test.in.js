...
function addSomePokemonsToThePlayersParty(
  region = GameConstants.MAX_AVAILABLE_REGION
) {
  let pokemons = [];
...
function resetSettings() {
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.PrioritizedSortingDescending,
    true
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.NotShinyFirst,
    false
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.NotAlternateFormFirst,
    false
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.SpreadPokerus,
    true
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.PrioritizedRegion,
    GameConstants.Region.none
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.PrioritizedType,
    PokemonType.None
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.RegionalDebuffRegion,
    GameConstants.Region.none
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.PrioritizedSorting,
    SortOptions.breedingEfficiency
  );
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.PrioritizedCategory,
    -1
  );
}
...
beforeAll(() => {
  ...
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.UseEggs,
    true
  );
  expect(
    Automation.Utils.LocalStorage.getValue(Automation.Hatchery.Settings.UseEggs)
  ).toBe("true");

  // Simulate UseFossils setting being enabled by default (done in the Automation.InitSteps.BuildMenu)
  Automation.Utils.LocalStorage.setValue(
    Automation.Hatchery.Settings.UseFossils,
    true
  );
  expect(
    Automation.Utils.LocalStorage.getValue(
...
test("Hatching any egg that completed its breeding period", () => {
  ...
  App.game.breeding.__eggList[0].__steps =
    App.game.breeding.__eggList[0].totalSteps;
...
describe(`${AutomationTestUtils.categoryPrefix}Egg breeding`, () => {
  ...
  let possibleEggs = Object.keys(GameConstants.EggItemType).filter(
    (eggType) => isNaN(eggType) && eggType != "Pokemon_egg"
  );
  test.each(possibleEggs)(
    "Having a %s that can hatch uncaught pokemon",
    (eggType) => {
      ...
      let expectedPokemonType =
        eggType == "Mystery_egg" ? EggType.Fire : expectedEggType;

      expect(App.game.breeding.__eggList[0].pokemon).toEqual(
        App.game.breeding.hatchList[expectedPokemonType][0][0]
      );
      expect(App.game.breeding.__eggList[0].isNone()).toBe(false);
  ...
  test("Don't breed multiple eggs of the same type", () => {
    ...
    expect(App.game.breeding.__eggList[0].pokemon).toEqual(
      App.game.breeding.hatchList[EggType.Fire][0][0]
    );
    expect(App.game.breeding.__eggList[0].isNone()).toBe(false);
    ...
    expect(App.game.breeding.__eggList[1].pokemon).toEqual(
      App.game.breeding.hatchList[EggType.Grass][0][0]
    );
    expect(App.game.breeding.__eggList[1].isNone()).toBe(false);
    ...
    expect(App.game.breeding.__eggList[0].pokemon).toEqual(
      App.game.breeding.hatchList[EggType.Fire][0][1]
    );
    expect(App.game.breeding.__eggList[0].isNone()).toBe(false);
    expect(App.game.breeding.__eggList[0].type).toBe(EggType.Fire);
  });
...
describe(`${AutomationTestUtils.categoryPrefix}Party pokémon breeding`, () => {
  ...
  test("Breed lvl 100 pokémons", () => {
    ...
    expect(App.game.breeding.__eggList[0].pokemon).toEqual(
      App.game.party.caughtPokemon[0].name
    );
    expect(App.game.breeding.__eggList[0].isNone()).toBe(false);
  });
  ...
  test("Test breeding order with pokérus > SpreadPokerus setting disabled", () => {
    ...
    App.game.party.getPokemon(pokemonMap["Mankey"].id).pokerus =
  ...
  test("Test breeding order with pokérus when not enough pokémon share type with the infected one", () => {
    ...
    for (const pokemon of App.game.party.caughtPokemon) {
      if (
        pokemonMap[pokemon.id].type.includes(PokemonType.Fighting) &&
        pokemon.name != "Machop"
      ) {
    ...
    App.game.party.getPokemon(pokemonMap["Squirtle"].id).pokerus =
      GameConstants.Pokerus.Contagious;

    // Complete Pikachu and Qwilfish steps
    App.game.breeding.__eggList[2].__steps =
      App.game.breeding.__eggList[2].totalSteps;
    App.game.breeding.__eggList[3].__steps =
      App.game.breeding.__eggList[3].totalSteps;
...
describe(`${AutomationTestUtils.categoryPrefix}Party pokémon breeding order > Region priority`, () => {
  ...
  let regionTestCases = [
    {
      regionId: GameConstants.Region.kanto,
      regionName: "Kanto",
      expectedPokemons: ["Pikachu", "Hitmonlee", "Ponyta", "Hitmonchan"],
    },
    {
      regionId: GameConstants.Region.johto,
      regionName: "Johto",
      expectedPokemons: ["Qwilfish", "Magby", "Totodile", "Elekid"],
    },
    {
      regionId: GameConstants.Region.hoenn,
      regionName: "Hoenn",
      expectedPokemons: ["Mudkip", "Clamperl", "Torchic", "Numel"],
    },
    {
      regionId: GameConstants.Region.sinnoh,
      regionName: "Sinnoh",
      expectedPokemons: ["Pachirisu", "Carnivine", "Turtwig", "Buizel"],
    },
    {
      regionId: GameConstants.Region.unova,
      regionName: "Unova",
      expectedPokemons: ["Sawk", "Throh", "Tepig", "Blitzle"],
    },
    {
      regionId: GameConstants.Region.kalos,
      regionName: "Kalos",
      expectedPokemons: ["Chespin", "Froakie", "Fennekin", "Goomy"],
    },
    {
      regionId: GameConstants.Region.alola,
      regionName: "Alola",
      expectedPokemons: ["Litten", "Crabrawler", "Turtonator", "Rowlet"],
    },
  ];

  // Test the breeding order with infected and the PrioritizedRegion setting enabled
  test.each(regionTestCases)(
    "Test breeding order with region set to $regionName",
    (testCase) => {
      ...
      for (const i of testCase.expectedPokemons.keys()) {
        expect(App.game.breeding.__eggList[i].pokemon).toEqual(
          testCase.expectedPokemons[i]
        );
...
describe(`${AutomationTestUtils.categoryPrefix}Party pokémon breeding order > Type priority`, () => {
  ...
  let regionTestCases = [
    {
      typeId: PokemonType.Normal,
      typeName: "Normal",
      expectedPokemons: ["Miltank", "Pidgey", "Drampa", "Pidgeotto"],
    },
    {
      typeId: PokemonType.Fire,
      typeName: "Fire",
      expectedPokemons: ["Litten", "Ponyta", "Turtonator", "Magby"],
    },
    {
      typeId: PokemonType.Water,
      typeName: "Water",
      expectedPokemons: ["Gyarados", "Qwilfish", "Popplio", "Mudkip"],
    },
    {
      typeId: PokemonType.Electric,
      typeName: "Electric",
      expectedPokemons: ["Raichu", "Pikachu", "Pachirisu", "Electabuzz"],
    },
    {
      typeId: PokemonType.Grass,
      typeName: "Grass",
      expectedPokemons: ["Carnivine", "Bellsprout", "Rowlet", "Turtwig"],
    },
    {
      typeId: PokemonType.Ice,
      typeName: "Ice",
      expectedPokemons: ["Piloswine", "Cloyster", "Dewgong", "Lapras"],
    },
    {
      typeId: PokemonType.Fighting,
      typeName: "Fighting",
      expectedPokemons: ["Sawk", "Throh", "Hitmonlee", "Hitmonchan"],
    },
    {
      typeId: PokemonType.Ground,
      typeName: "Ground",
      expectedPokemons: ["Geodude", "Rhydon", "Rhyhorn", "Dugtrio"],
    },
    {
      typeId: PokemonType.Flying,
      typeName: "Flying",
      expectedPokemons: ["Gyarados", "Scyther", "Murkrow", "Archen"],
    },
    {
      typeId: PokemonType.Psychic,
      typeName: "Psychic",
      expectedPokemons: ["Exeggutor", "Slowbro", "Starmie", "Xatu"],
    },
    {
      typeId: PokemonType.Bug,
      typeName: "Bug",
      expectedPokemons: ["Scyther", "Ariados", "Anorith", "Forretress"],
    },
    {
      typeId: PokemonType.Rock,
      typeName: "Rock",
      expectedPokemons: ["Geodude", "Rhydon", "Rhyhorn", "Cranidos"],
    },
    {
      typeId: PokemonType.Ghost,
      typeName: "Ghost",
      expectedPokemons: ["Gastly", "Haunter", "Gengar", "Gyarados"],
    },
    {
      typeId: PokemonType.Dragon,
      typeName: "Dragon",
      expectedPokemons: ["Turtonator", "Drampa", "Tyrunt", "Kingdra"],
    },
    {
      typeId: PokemonType.Dark,
      typeName: "Dark",
      expectedPokemons: ["Murkrow", "Houndour", "Houndoom", "Deino"],
    },
    {
      typeId: PokemonType.Steel,
      typeName: "Steel",
      expectedPokemons: ["Forretress", "Steelix", "Magnemite", "Shieldon"],
    },
    {
      typeId: PokemonType.Fairy,
      typeName: "Fairy",
      expectedPokemons: ["Clefairy", "Marill", "Mr. Mime", "Morelull"],
    },
  ];

  // Test the breeding order with infected and the PrioritizedType setting enabled
  test.each(regionTestCases)(
    "Test breeding order with type set to $typeName",
    (testCase) => {
      ...
      for (const i of testCase.expectedPokemons.keys()) {
        ...
        let expectMatchingType = !(
          i === 3 && testCase.typeId === PokemonType.Ghost
        );
        expect(pokemeonData.type.includes(testCase.typeId)).toBe(
          expectMatchingType
        );
      }
    }
  ...
  let regionTestCases = [
    {
      regionId: GameConstants.Region.kanto,
      regionName: "Kanto",
      expectedPokemons: ["Pikachu", "Hitmonlee", "Sawk", "Ponyta"],
    },
    {
      regionId: GameConstants.Region.johto,
      regionName: "Johto",
      expectedPokemons: ["Qwilfish", "Sawk", "Pikachu", "Magby"],
    },
    {
      regionId: GameConstants.Region.hoenn,
      regionName: "Hoenn",
      expectedPokemons: ["Sawk", "Pikachu", "Mudkip", "Throh"],
    },
    {
      regionId: GameConstants.Region.sinnoh,
      regionName: "Sinnoh",
      expectedPokemons: ["Pachirisu", "Sawk", "Carnivine", "Pikachu"],
    },
    {
      regionId: GameConstants.Region.unova,
      regionName: "Unova",
      expectedPokemons: ["Sawk", "Throh", "Pikachu", "Hitmonlee"],
    },
    {
      regionId: GameConstants.Region.kalos,
      regionName: "Kalos",
      expectedPokemons: ["Sawk", "Pikachu", "Throh", "Hitmonlee"],
    },
    {
      regionId: GameConstants.Region.alola,
      regionName: "Alola",
      expectedPokemons: ["Sawk", "Litten", "Crabrawler", "Turtonator"],
    },
  ];

  // Test the breeding order with infected and the SpreadPokerus setting enabled
  test.each(regionTestCases)(
    "Test breeding order with regional debuff set to $regionName",
    (testCase) => {
      addSomePokemonsToThePlayersParty();
...
describe(`${AutomationTestUtils.categoryPrefix}Party pokémon breeding order > Category priority`, () => {
  ...
  test.each(categoryTestCases)(
    "Test breeding order with category set to $categoryName",
    (testCase) => {
      addSomePokemonsToThePlayersParty();
