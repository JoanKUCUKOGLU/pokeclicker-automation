...
function expectMapValueToBeUpdatedForPokemon(pokemon) {
  let valueBefore = Automation.Utils.Battle.__internal__PokemonAttackMap.get(
    pokemon.id
  );
  ...
  let valueAfter = Automation.Utils.Battle.__internal__PokemonAttackMap.get(
    pokemon.id
  );
...
describe(`${AutomationTestUtils.categoryPrefix}Check pokemon attack internal map`, () => {
  ...
  test("Check pokemon attack map initialization", () => {
    ...
    expect(Automation.Utils.Battle.__internal__PokemonAttackMap.size).toEqual(
  ...
  test("Check pokemon attack map update => effortPoints changed", () => {
    ...
    let newValue = Automation.Utils.Battle.__internal__PokemonAttackMap.get(
      pokemon.id
    );
  ...
  test("Check pokemon attack map does not update more than once every second", () => {
    ...
    // The value should have been updated
    valueAfter = Automation.Utils.Battle.__internal__PokemonAttackMap.get(
      pokemon.id
    );
    expect(valueAfter).not.toEqual(valueBefore);
  });
...
test("Check calculatePokemonAttack() output with weather change", () => {
  ...
  let resultWithNoWeather = Automation.Utils.Battle.calculatePokemonAttack(
    pokemonType,
    region,
    WeatherType.Clear
  );
  ...
  let result = Automation.Utils.Battle.calculatePokemonAttack(
    pokemonType,
    region,
    WeatherType.Sunny
  );
...
describe(`${AutomationTestUtils.categoryPrefix}Check getGameTickCountNeededToDefeatPokemon() method`, () => {
  let testCases = [
    {
      testDesc:
        "Click attack high enough to defeat pokemon in a single click attack",
      clickAttack: 10000,
    ...
    {
      testDesc:
        "Click attack high enough to defeat pokemon in a 3 click attacks",
      clickAttack: 4000,
      pokemonAtkPerSecond: 8000,
      expectedResult: 3,
    },
    {
      testDesc:
        "Click attack not high enough to defeat pokemon without pokemons",
      clickAttack: 195,
    ...
    {
      testDesc:
        "Both click and pokemons attack not high enough to defeat pokemon in a single turn",
      clickAttack: 20,
  ...
  test.each(testCases)("$testDesc", (testCase) => {
    let totalAtkPerSecond =
      testCase.clickAttack * 20 + testCase.pokemonAtkPerSecond;
    let pokemonHp = 10000;
    let result = Automation.Utils.Battle.getGameTickCountNeededToDefeatPokemon(
      pokemonHp,
      testCase.clickAttack,
      totalAtkPerSecond
    );
...
describe(`${AutomationTestUtils.categoryPrefix}Check getPlayerWorstPokemonAttack() method`, () => {
  ...
  test.each(
    Array(GameConstants.MAX_AVAILABLE_REGION + 1)
      .fill()
      .map((_, i) => i)
  )("Check getPlayerWorstPokemonAttack for region %d", (regionId) => {
